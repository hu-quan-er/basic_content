...
| 本文负责 | 本文不展开 |
|---|---|
| 生产级工具平台：工具注册、发现、权限、schema/语义校验、risk/HITL、幂等、沙箱、审计、测试和版本治理 | Function Calling 基础见 [03-工具调用.md](./03-工具调用.md)；MCP 协议见 [03.1-MCP.md](./03.1-MCP.md)；结构化输出与约束解码算法见 [22-结构化输出与约束解码专题.md](./22-结构化输出与约束解码专题.md)；Agent 全生命周期安全治理见 [27-Agent安全生命周期专题.md](./27-Agent安全生命周期专题.md) |
