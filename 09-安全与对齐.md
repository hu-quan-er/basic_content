...
| 本文负责 | 本文不展开 |
|---|---|
| Prompt Injection、Jailbreak、数据安全、Excessive Agency、Guardrails、HITL、对齐相关概念的安全总览 | Agent 全生命周期安全治理见 [27-Agent安全生命周期专题.md](./27-Agent安全生命周期专题.md)；工具执行安全见 [23-Tool Gateway与工具平台专题.md](./23-Tool%20Gateway与工具平台专题.md)；模型调用安全见 [26-Model Gateway与模型治理专题.md](./26-Model%20Gateway与模型治理专题.md) |
...
这些风险在 Agent 生命周期中的位置、防护实现和测试方法，见 [27-Agent安全生命周期专题.md](./27-Agent安全生命周期专题.md)。
